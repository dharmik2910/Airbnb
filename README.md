...
A desktop-only, production-structured clone of an Airbnb listing page built with
Next.js 14 (App Router), React 18, TypeScript, and Tailwind CSS.

## Important note on the reference site

The reference (`https://airbnb-clone-umber-two.vercel.app`) blocks automated
fetch/scrape access (`robots.txt` disallow), and the task instructions
explicitly prohibit lift-and-shift of its codebase. This submission was
therefore built from the well-documented, publicly known layout and
interaction patterns of a standard Airbnb listing page (hero photo grid,
photo tour, single-photo lightbox, sticky reserve card, amenities, reviews,
host section, map), reproduced independently in original code. If you can
share direct screenshots or an export of the reference, a follow-up pass with
the `ui-fidelity-reviewer` agent (see `/agents`) would tighten pixel-level
...
- **Tailwind CSS** for styling (no component library — all custom, matching
  Airbnb's own approach of a bespoke design system)
- No backend — listing data is a typed mock module (`data/listing.ts`); this
  keeps the take-home focused per the instructions ("backend is optional").
...
## Additional sections (closing UI gaps vs. a real listing page)

Beyond the original scope, the page now includes every major module a real
Airbnb listing page has:

- **Interactive calendar** (`Calendar.tsx`) — real two-month date picker,
  click check-in then check-out, range highlighting, past dates disabled,
  wired into both the desktop reserve card and the mobile sheet so pricing
  (`nights x price`) updates live instead of using a static placeholder date.
...
data, or literal copy was reproduced; see the note at the top of this file
for why.
...
1. **Listing page** (`/`) — full page composed from the components above.
2. **Photo Tour** — opened via "Show all photos" or the large hero photo.
   Full-screen scrollable grid of every photo, each of which opens the
...
3. **Lightbox** — opened from any thumbnail in the hero grid or the Photo
   ...
   navigation, thumbnail strip with active-state highlighting, `Escape` to
   close, focus is sent to the close button on open.

## Responsive behavior

Built mobile-first with Tailwind breakpoints (`sm`, `md`, `lg`), matching the
real Airbnb site's responsive patterns:
...
- Both overlays use `role="dialog"` + `aria-modal="true"`, move focus to the
  close button on open, and restore body scroll on close.
- All icon-only buttons carry `aria-label`.
- Keyboard support: `Escape` closes both overlays; `ArrowLeft`/`ArrowRight`
  navigate the Lightbox; guest stepper buttons disable at min/max and expose
  their state via `disabled`.
- Visible focus rings (`:focus-visible`) matching Airbnb's dark outline style
  are preserved on every interactive element rather than suppressed.
